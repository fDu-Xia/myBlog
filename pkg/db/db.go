...
type MySQLOptions struct {
	...
	MaxIdleConnections    int           `json:"max-idle-connections"`    // 最大空闲连接数
	MaxOpenConnections    int           `json:"max-open-connections"`    // 最大打开连接数
	MaxConnectionLifeTime time.Duration `json:"max-connection-lifetime"` // 连接的最大生命周期
	LogLevel              int           `json:"log-level"`               // 日志级别
}
